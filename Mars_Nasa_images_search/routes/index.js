...
async function create_user(req, res) {
    //generate a random 10 chars string
...
router.post('/nasaPage', function(req, res, next){
    return db.Users.findOne({ where: { email: req.body.email } }).then(async (uname) => {
        if (await bcrypt.compare(req.body.password, uname.password)) {
            req.session.email = req.body.email.trim();
            req.session.logIn = true;
            res.render('nasaPage', { title: '', userName: uname.firstName, email: uname.email });
        }
        else
            res.render('login', { title: 'Login', errorMail: '',registerMsg:'Password is incorrect'});
    }).catch((err)=>{
        res.render('login', { title: 'Login', errorMail: '',registerMsg:'Mail is incorrect'});
    });
});
